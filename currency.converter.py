...
lang = params.get("lang", ["en"])[0][:2].lower() if params.get("lang") else "en"
...
# Language selector with hidden accessibility label
st.write("<div style='float:right'>", unsafe_allow_html=True)
new_lang = st.selectbox(
    "Language selector",  # Accessible label
    options=list(LANGUAGES.keys()),
    format_func=lambda x: x.upper(),
    index=list(LANGUAGES.keys()).index(lang),
    label_visibility="hidden"  # Hidden visually but accessible
)
if new_lang != lang:
